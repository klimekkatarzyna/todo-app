...
lists.get("/getLists/:invitationToken", async (req, res) => {
  const userId = getSessionUserId(req);
  if (!userId) return;
...
lists.patch("/addInvitationTokenToList", async (req, res) => {
  List.updateMany(
    { _id: req.body.listId },
    {
      $set: {
...
lists.patch("/addUserToMemberOfList", async (req, res) => {
...
lists.patch("/removeMemberFromList", async (req, res) => {
  List.findOneAndUpdate(
    { _id: req.body.listId },
...
lists.patch("/removeInvitation", async (req, res) => {
  List.findOneAndUpdate(
    { _id: req.body.listId },
    { $set: { members: [], invitationToken: "" } },
    (err: unknown, docs: unknown) => {
      try {
        res.json({
          message: `member removed successfully to ${req.body.member}`,
